...
def compute_reward(observation_raw, dt, num_joints, params, feet_status, all_torques, action_buffer, is_obs_fullstate,
    ...
    forward_vel_reward = params.get("weight_velocity", 0) * (1 - abs(observation.local_vel[2] - cmd_fwd_vel) / cmd_fwd_vel)

    torque_reward =  -params.get("weight_torque", 0) * squared_norm(torque)

    smoothness1_reward = -params.get("weight_smoothness1", 0) * squared_norm(action_dot)

    smoothness2_reward = -params.get("weight_smoothness2", 0) * squared_norm(action_ddot)
    # print((1 - 0.015187685621055103/params.get("weight_velocity", 0))*cmd_fwd_vel+cmd_fwd_vel)
    # exit()
